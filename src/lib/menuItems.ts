...
function menuClient(client?: SupabaseClient) {
...
export async function fetchPublicMenuItems(slug: MenuSlug, client?: SupabaseClient): Promise<MenuItem[]> {
  const { data, error } = await menuClient(client)
...
export async function fetchAllPublicMenuItems(client?: SupabaseClient): Promise<DbMenuItem[]> {
  const { data, error } = await menuClient(client)
    .from('savannah_menu_items')
    .select('*')
    .eq('is_available', true)
...
export async function fetchMenuCategories(client?: SupabaseClient): Promise<DbMenuCategory[]> {
  const { data, error } = await menuClient(client)
...
export async function createMenuCategory(input: MenuCategoryInput) {
  const { error } = await getSupabase().from('savannah_menu_categories').insert(input);
  if (error) throw error;
}

export async function updateMenuCategory(id: string, input: MenuCategoryInput) {
  const { error } = await getSupabase().from('savannah_menu_categories').update(input).eq('id', id);
  if (error) throw error;
}

export async function deleteMenuCategory(id: string) {
  const { error } = await getSupabase().from('savannah_menu_categories').delete().eq('id', id);
  if (error) throw error;
}
...
export async function createMenuItem(input: MenuItemInput) {
  const { error } = await getSupabase().from('savannah_menu_items').insert(input);
  if (error) throw error;
}

export async function updateMenuItem(id: string, input: MenuItemInput) {
  const { error } = await getSupabase().from('savannah_menu_items').update(input).eq('id', id);
  if (error) throw error;
}

export async function deleteMenuItem(id: string) {
  const { error } = await getSupabase().from('savannah_menu_items').delete().eq('id', id);
  if (error) throw error;
}
